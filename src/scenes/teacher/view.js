...
const ViewTeacher = () => {
  ...
  const handleDelClick = async ({ _id }) => {
    try {
      ...
      const response = await axios.delete(
        `${API_URLS}/admin/teacher-menu/${_id}`,
        config
      );
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${API_URLS}/admin/teacher-menu`);
  ...
  const columns = [
    ...
    { field: "features", headerName: "Features", flex: 6 },
    { field: "buttons", headerName: "Buttons", flex: 1 },
    {
      ...
      renderCell: (params) => (
        ...
        >
          <Link
            to={`/teacher-menu/edit/${params.row._id}
